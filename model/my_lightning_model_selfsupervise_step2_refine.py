...
# from .self_supervise_loss import consistensy_loss, peaky_loss
from .self_supervise_loss_step2_refine import consistensy_peaky_loss
...
class CornerDetectionLightningModel(pl.LightningModule):
    ...
    def compute_loss(self, frame, events, corner_mask, reset_mask,
        ...
        consistensy_loss, peaky_loss = consistensy_peaky_loss(pred,
                                                         rotation_vectors, translation_vectors,
                                                         camera_nts, camera_depths,
                                                         camera_Ks, camera_Kinvs,
                                                         origin_sizes, (self.hparams.height, self.hparams.width),
                                                         # event_mask=torch.sum(events.float(), dim=2, keepdim=True))
                                                         event_mask=None)

        loss_dict["consistensy_loss"] = self.consistensy_loss_alpha * consistensy_loss
        loss_dict["peaky_loss"] = self.peaky_loss_alpha * peaky_loss
    ...
    def training_step(self, batch, batch_nb):
        ...
        self.log('learning_rate', learning_rate)  ## 打印学习率
        self.log('peaky_loss_alpha', self.peaky_loss_alpha)  ## 打印alpha
    ...
    def configure_optimizers(self):
        ...
        # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer, milestones=[10, 15, 20, 25], gamma=0.5, verbose=True)
