...
from model.my_lightning_model_selfsupervise_step2_refine import CornerDetectionCallback, CornerDetectionLightningModel
...
def main(raw_args=None):
    ...
    # dir params
    parser.add_argument('--root_dir', type=str, default='./train_logs_selfsupervise_step2_refine/', help='logging directory')
    ...
    parser.add_argument('--checkpoint', type=str, default='./train_logs_selfsupervise_step1/.ckpt', help='resume from specific checkpoint')
    ...
    parser.add_argument('--data_device', type=str, default='ccheckpointuda:0', help='run simulation on the cpu/gpu')
    ...
    parser.add_argument('--raw_number_of_heatmaps_list', type=int, default=(19, 46, 82),
                        help='number of raw corner heatmaps list')
    parser.add_argument('--needed_number_of_heatmaps', type=int, default=10, help='number of target corner heatmaps')
    parser.add_argument('--min_blur_ratio', type=float, default=0.2,
                        help='min ratio for clear images to generate blurred image')
    parser.add_argument('--max_blur_ratio', type=float, default=0.6,
                        help='max ratio for clear images to generate blurred image')
    parser.add_argument('--random_gamma_transform', default=True,
                        help='Whether to perform random gamma transform on image intensity')
    parser.add_argument('--random_pepper_noise', default=True,
                        help='Whether to add pepper noise on event representation')
    ...
    parser.add_argument('--reload_dataloaders_every_n_epochs', type=int, default=1,
                        help='Set to a non-negative integer to reload dataloaders every n epochs')  # TODO: test
